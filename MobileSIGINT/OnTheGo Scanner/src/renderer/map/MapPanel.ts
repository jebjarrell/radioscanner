...
import { DEFAULT_MAP_STYLE_ID, resolveMapStyleUrl } from '../utils/mapStyles.js';
...
export class MapPanel {
  ...
  private currentMapStyleId: string;
  ...
  constructor(root: HTMLElement, options: MapPanelOptions) {
    ...
    this.currentMapStyleId = options.mapStyleId ?? DEFAULT_MAP_STYLE_ID;
  ...
  public setMapStyle(styleId: string): void {
    if (styleId === this.currentMapStyleId) {
      return;
    }
    this.currentMapStyleId = styleId;
    this.mapReady = false;
    this.map.setStyle(resolveMapStyleUrl(styleId));
    // Re-sync overlays once the new style has finished loading; setStyle drops
    // any custom sources/markers we added on top of the previous style.
    this.map.once('load', () => {
      this.mapReady = true;
      this.refreshLayers();
      if (this.layerState.drones && this.currentDrones.length) {
  ...
  private createMap(): MapLibreMap {
    const map = new MapLibreMap({
      ...
      style: resolveMapStyleUrl(this.currentMapStyleId),

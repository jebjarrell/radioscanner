...
const state: { gpsManualFallback: ManualGpsFallback } = {
  gpsManualFallback: {
    enabled: false,
    lat: null,
    lon: null,
...
export function getManualGpsFallback(): ManualGpsFallback {
  return { ...state.gpsManualFallback };
}

export function setManualGpsFallback(
  input: Partial<Omit<ManualGpsFallback, 'updatedAt'>> & { updatedAt?: number },
): void {

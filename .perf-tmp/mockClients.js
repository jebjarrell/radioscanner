...
import { ALTITUDE_MAX_FT, ALTITUDE_MIN_FT, HEADING_MAX_DEG, HEADING_MIN_DEG, LAT_MAX, LAT_MIN, LON_MAX, LON_MIN, SPEED_MAX_KTS, SPEED_MIN_KTS, } from './constants.js';
import { getManualGpsFallback } from './settings.js';
import { TypedEventEmitter } from './utils/typedEventEmitter.js';
export class MockDump1090Client extends TypedEventEmitter {
    timer = null;
    status = { healthy: true, lastUpdated: Date.now() };
    lastSnapshot = null;
    aircraft = new Map();
    start() {
        if (this.timer)
...
export class MockRtlTcpClient extends TypedEventEmitter {
...
export class MockKismetClient extends TypedEventEmitter {
    timer = null;
    status = { available: true, ridEnabled: true, lastChecked: Date.now() };
    start() {
        if (this.timer)
            return;
...
export class MockGpsClient extends TypedEventEmitter {
    status = { connected: false, lastFix: null, lastChecked: undefined };
    timer = null;
    async start() {
        if (this.status.connected) {
            return;
